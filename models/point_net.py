...
# copy from https://github.com/itberrios/3D/blob/main/point_net/point_net.py
...
class PointNetBackbone(nn.Module):
    def __init__(self, num_points=2500, num_global_feats=1024, local_feat=True):
        ...
        self.tnet1 = Tnet(input_dim=3, num_points=num_points)
        self.tnet2 = Tnet(input_dim=64, num_points=num_points)
...
class PointNetClassHead(nn.Module):
    def __init__(self, num_points=2500, num_global_feats=1024, class_num=2):
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        x, crit_idx, A_feat = self.backbone(x)
...
class PointNetSegHead(nn.Module):
    def __init__(self, num_points=2500, num_global_feats=1024, class_num=2):
        ...
        self.backbone = PointNetBackbone(num_points=num_points, num_global_feats=num_global_feats, local_feat=True)

        num_features = num_global_feats + 64
        self.conv1 = nn.Conv1d(num_features, 512, 1)
        self.conv2 = nn.Conv1d(512, 256, 1)
        self.conv3 = nn.Conv1d(256, 128, 1)
        self.conv4 = nn.Conv1d(128, class_num, 1)
