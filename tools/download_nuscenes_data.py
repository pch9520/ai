...
useremail = os.getenv("nuscenes_user_email", "")
password = os.getenv("nuscenes_password", "")
assert useremail != "", "lack user email!!!"
assert password != "", "lack password!!!"

output_dir = "/Volumes/PS2000W/data/nuscenes"
...
def download_file(url, save_file, md5):
    ...
    progress_bar = tqdm(total=file_size, unit='B',
                        unit_scale=True, unit_divisor=1024, desc=save_file, ascii=True)
    ...
    hash = md5obj.hexdigest()
    assert hash == md5, f"md5 check failed, {save_file}"
...
def extract_tgz_to_original_folder(tgz_file_path):
    ...
    tqdm.write(f"Extracting {tgz_file_path} to {original_folder}")

    with gzip.open(tgz_file_path, 'rb') as f_in:
        with tarfile.open(fileobj=f_in, mode='r') as tar:
            for member in tqdm(iterable=tar.getmembers(), total=len(tar.getmembers()), desc=tgz_file_path):
                tar.extract(member, path=original_folder)


def extract_tar_to_original_folder(tar_file_path):
    original_folder = os.path.dirname(tar_file_path)
    tqdm.write(f"Extracting {tar_file_path} to {original_folder}")

    with tarfile.open(tar_file_path, 'r') as tar:
        for member in tqdm(iterable=tar.getmembers(), total=len(tar.getmembers()), desc=tar_file_path):
            tar.extract(member, path=original_folder)
...
def main():
    ...
    with ThreadPoolExecutor(max_workers=3) as executor:
        for output_name, (download_url, save_file, md5) in download_data.items():
            futures.append(executor.submit(download_file, download_url, save_file, md5))
            input_params.append(output_name)
    for idx, future in enumerate(as_completed(futures)):
        save_file = future.result()
        output_name = input_params[idx]
        download_data[output_name][1] = save_file

    print("Extracting files...")
    futures.clear()
    with ThreadPoolExecutor(max_workers=5) as executor:
        for _, (download_url, save_file, md5) in download_data.items():
            if save_file.endswith(".tgz"):
                futures.append(executor.submit(extract_tgz_to_original_folder, save_file))
            elif save_file.endswith(".tar"):
                futures.append(executor.submit(extract_tar_to_original_folder, save_file))
            else:
                tqdm.write("unknow file type", save_file)
